...
public class Minotaur extends BaseHostileEntity implements Fighter {
	 Rectangle minotaur;
	    int width, height;
	    Animation stand;

	    public Minotaur(Handler handler, int xPosition, int yPosition, String state, String name, String area, BufferedImage[] animFrames) {
	        ...
	        stand = new Animation (400, Images.minotaur);
	        this.foundState = state;
	        minotaur = new Rectangle();
	    }
	    ...
	    public void render(Graphics g) {
	        ...
	        if(handler.getArea().equals(this.Area)) {
	            if (!Player.checkInWorld) {
	                minotaur = new Rectangle((int) (handler.getXDisplacement() + getXOffset()),
	                        (int) (handler.getYDisplacement() + getYOffset()), 45, 45);

	            } else {
	                minotaur = new Rectangle((int) (handler.getXInWorldDisplacement() + getXOffset()),
	            ...
	            g.drawImage(stand.getCurrentFrame(),minotaur.x,minotaur.y,minotaur.width,minotaur.height,null);

	            if (minotaur.intersects(handler.getEntityManager().getPlayer().getCollision())) {
